...
function populateProblemsDropdown() {
    ...
    elements.problemSelect.innerHTML = '<option value="">Select a problem...</option>';
    ...
    console.log('DEBUG: Final dropdown innerHTML:', elements.problemSelect.innerHTML);
...
const problemDescriptions = {
    ...
    "three-sum": {
        ...
        outputFormat: "Print the triplets in the format [[a,b,c],[d,e,f]] or [] if no triplets found",
        constraints: [
            "0 ≤ n ≤ 3000",
            "-10^5 ≤ nums[i] ≤ 10^5"
        ],
    ...
    "binary-search-insert-position": {
        ...
        inputFormat: "First line: sorted array in format [1,3,5,6]\nSecond line: target integer",
        ...
        examples: [
            {
                input: "2\n1\n1 2",
                output: "2",
                explanation: "Person 1 trusts person 2, and person 2 trusts nobody, so person 2 is the judge"
            }
        ]
    },
    "front-middle-back-queue": {
        title: "Design Front Middle Back Queue",
        description: "Design a queue that supports push and pop operations in the front, middle, and back. Implement the FrontMiddleBack class with various operations like pushFront, pushMiddle, pushBack, popFront, popMiddle, popBack.",
    ...
    "merge-sort": {
        ...
        inputFormat: "First line: number of elements\nSecond line: space-separated integers",
    ...
    "merge-two-sorted-lists": {
        ...
        inputFormat: "First line: first sorted list values\nSecond line: second sorted list values",
        ...
        elements.problemDetails.innerHTML = `
            <div class="problem-header">
                ...
                <div class="problem-meta">
                    ...
                    <span class="test-count-badge">📝 ${(problemData.public_tests?.length || 0) + (problemData.hidden_tests?.length || 0)} Test Cases</span>
                </div>
            </div>
            ...
            <div class="problem-content">
                ...
                <div class="section-block">
                    <h5>📤 Output Format</h5>
                    <p class="format-description">${problemInfo.outputFormat}</p>
                </div>

                <div class="section-block">
                    <h5>⚡ Constraints</h5>
                    <ul class="constraints-list">
                        ${problemInfo.constraints.map(constraint => `<li>${constraint}</li>`).join('')}
                    </ul>
                </div>
    ...
    if (problemData.public_tests && problemData.public_tests.length > 0) {
        const sampleHtml = `
            <div class="sample-tests-section">
                ...
                ${problemData.public_tests.slice(0, 3).map((test, index) => `
                    <div class="test-case">
                ...
                ${problemData.public_tests.length > 3 ? 
                    `<div class="more-tests-note">
                        <p><em>💼 ${problemData.public_tests.length - 3} more public test cases + ${problemData.hidden_tests?.length || 0} hidden test cases</em></p>
                    </div>` : ''}
            </div>
        `;
...
async function submitSolution() {
    ...
    try {
        ...
        hideLoading();
        setSubmitButtonLoading(false);
        showError('Failed to submit solution. Please check your code and try again.');
    }
}
    ...
    const resultHtml = `
        <div class="result-card ${statusClass}">
            ...
            <div class="result-details">
                <strong>Problem:</strong> ${formatProblemName(currentProblem)}<br>
...
function showSuccessModal(result) {
    ...
    let modalContent = `
        <div class="text-center">
            ...
            <p><strong>Status:</strong> ${grade.replay_result === 'passed' ? 'Perfect Solution! 🎉' : 
                grade.replay_result === 'partially' ? 'Good Progress! 🔄' : 'Keep Trying! 💪'}</p>
        </div>
    `;
